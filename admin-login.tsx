...
export default function AdminLogin({ onLogin, onBack }: AdminLoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const admin = existingAdmins.find((a: any) => a.email === formData.email && a.password === formData.password)

    if (admin) {
      onLogin(admin, "admin")
    } else {
      // For demo purposes, allow new admin registration
      const newAdmin = { ...formData, id: Date.now() }
      existingAdmins.push(newAdmin)
      localStorage.setItem("admins", JSON.stringify(existingAdmins))
      onLogin(newAdmin, "admin")
    }
  }
